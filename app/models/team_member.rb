...
class TeamMember < ApplicationRecord
  ...
  def add_to_linked_internal_chat_channels
    return unless team.account.feature_enabled?('internal_chat')
    ...
    InternalChat::ChannelTeam.where(team_id: team_id).find_each do |channel_team|
      channel_team.channel.channel_members.find_or_create_by!(user_id: user_id) { |membership| membership.role = :member }
    end
  end
