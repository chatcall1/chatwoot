class InternalChatListener < BaseListener
  ...
  def internal_chat_channel_updated(event)
    ...
    account = channel&.account || Account.find_by(id: event.data[:account_id])
    return if account.blank?

    tokens = event.data[:member_tokens] || member_tokens(channel)
    data = event.data[:channel_data] || channel_event_data(channel)
    broadcast(account, tokens, INTERNAL_CHAT_CHANNEL_UPDATED, data)
  end
  ...
  def channel_event_data(channel)
    {
      id: channel.id,
      name: channel.name,
